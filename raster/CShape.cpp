...
void CShape::Pentagon(CDC* pDC, int xc, int yc, int r, COLORREF color)
{
	...
	for (i = 0; i < 4; i++)
		CLine::DDALine(pDC, x[i], y[i], x[i + 1], y[i + 1], color);
	CLine::DDALine(pDC, x[4], y[4], x[0], y[0], color);
}
//***********************************************************************************
...
void CShape::Hexagon(CDC* pDC, int xc, int yc, int r, COLORREF color)
{
	...
	for (i = 0; i < 5; i++)
		CLine::DDALine(pDC, x[i], y[i], x[i + 1], y[i + 1], color);
	CLine::DDALine(pDC, x[5], y[5], x[0], y[0], color);
}
//***********************************************************************************
...
void CShape::Star(CDC* pDC, int xc, int yc, int R, COLORREF color, int n)
{
	...
	for (i = 0; i < 2 * n - 1; ++i)
		CLine::BresenhamLine(pDC, x[i], y[i], x[i + 1], y[i + 1], color);
	CLine::BresenhamLine(pDC, x[2 * n - 1], y[2 * n - 1], x[0], y[0], color);
}
